...
def batch_process(args):
    i, n_batch, node_ids, RNA_matrix, neighbor, cell_size = args
    gene_indices_all = []
    dic = {}
    if i < n_batch:
        for index, node in enumerate(node_ids[i * cell_size:(i + 1) * cell_size]):
            rna_ = RNA_matrix[:, node].todense()
            rna_[rna_ < 5] = 0
        ...
        for index, node in enumerate(node_ids[i * cell_size:]):
            rna_ = RNA_matrix[:, node].todense()
            rna_[rna_ < 5] = 0
            gene_indices = subgraph(RNA_matrix.transpose(), node, neighbor, np.squeeze(np.array(np.log(rna_[:, node].todense() + 1))))
            dic[node] = {'g': gene_indices}
            gene_indices_all = gene_indices_all + gene_indices
...
def initial_clustering(matrix, resolution=0.2, n_neighbors=15, custom_n_neighbors=None, n_pcs=40, custom_resolution=None, use_rep=None):
